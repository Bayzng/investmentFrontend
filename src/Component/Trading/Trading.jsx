...
//   // NEW: withdraw modal + wallet/amount state
...
//   // --- NEW: handle withdraw submit (sends wallet + amount to crypto.cryptofi@gmail.com)
...
//               <strong>Note:</strong> Ensure this is a CELO USDT-compatible address.
//               Incorrect addresses may lead to irreversible loss. Available balance:{" "}
//               <bold>{formatMoney(balance)}</bold>
//             </p>
...
export default function Trading() {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        setUsername(res.data.name || "");
        setBalance(typeof res.data.balance === "number" ? res.data.balance : 1000);
      } catch (error) {
  ...
  // --- Fetch user's trades when username available ---
  useEffect(() => {
    if (!username) return;
  ...
  const placeTrade = async () => {
    ...
    try {
      ...
      // Delay execution for tradeDelay seconds
      setTimeout(async () => {
        await executeTrade(savedTx._id);
        setTradeModalMessage("✅ Trade completed! Check your portfolio or transaction history below.");

        // Hide modal after 3s and optionally refresh
        setTimeout(() => {
          setShowTradeModal(false);
          // reload to reflect server-side balance if needed:
          window.location.reload();
        }, 3000);
      }, tradeDelay * 1000);
    } catch (err) {
  ...
  const executeTrade = async (tradeId) => {
    ...
    try {
      // Update local balance safely (note: backend is source of truth)
      setBalance((prevBalance) => {
        if (success) {
          return parseFloat((prevBalance + tx.amount + pnl).toFixed(2));
        }
      ...
      const res = await axios.put(
        `https://investmentbackend-6m5g.onrender.com/api/trades/${tradeId}`,
        {
          ...
          note,
          executedAt: new Date().toISOString(),
          balanceAfter: success ? parseFloat((balance + tx.amount + pnl).toFixed(2)) : balance,
        }
      );

      // update transaction in UI with returned data
      setTransactions((prev) => prev.map((t) => (t._id === tradeId ? res.data : t)));
    } catch (err) {
    ...
    <div className="trading-page">
      ...
      <div className="top-grid">
        ...
        <div className="trade-form-card small-card">
          ...
          <p style={{ marginTop: 10, fontSize: 12, color: "#666" }}>
            Trades are simulated. When executed, trades will randomly succeed or fail and update
      ...
      {showRequestModal && (
        <div className="tradingModal-overlay" onClick={() => setShowRequestModal(false)}>
          <div className="tradingModal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <textarea
              readOnly
              defaultValue={`To: crypto.cryptofi@gmail.com\nSubject: Request for Trading Tutor or Partner\n\nHello CryptoFi Team,\n\nI would like to request a trading ${balance <= 0 ? "tutor" : "partner"} to assist me in improving my trading strategies and maximizing my potential in the market.\n\nThank you,\n${username}`}
            />
            <p>
              ✉️ Copy and send to{" "}
      ...
      {showWithdrawModal && (
        <div className="withdrawModal-overlay" onClick={() => setShowWithdrawModal(false)}>
          <div className="withdrawModal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="withdraw-note">
              <strong>Note:</strong> Ensure this is a CELO USDT-compatible address. Incorrect addresses may lead to irreversible
              loss. Available balance: <address>{formatMoney(balance)}</address>
            </p>
            ...
            <div className="withdraw-actions">
              <button
                className="btn"
                onClick={handleWithdrawSubmit}
                disabled={!walletAddress || !withdrawAmount || !!withdrawError || Number(withdrawAmount) <= 0}
              >
                Submit Withdrawal
              </button>
